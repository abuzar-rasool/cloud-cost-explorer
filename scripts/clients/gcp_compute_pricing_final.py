...
PROJECT_ID = "hidden-moon-460018-n9"
...
def process_and_save_consolidated_data(skus, machines, output_file):
    ...
    try:
        ...
        with open(output_file, "w", newline='', encoding="utf-8") as f:
            ...
            writer.writeheader()
            print(f"Writing {len(joined_data)} records...")
            writer.writerows(joined_data)
...
def check_directory_permissions(dir_path):
    ...
    try:
        ...
        print(f"No write permission to directory {dir_path}: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if not check_directory_permissions(data_dir):
        print(f"Cannot write to data directory: {data_dir}")
        import sys
        sys.exit(1)
    
    # Define the output file path with timestamp
    timestamp = datetime.datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    output_file = os.path.join(data_dir, f"gcp_compute_pricing_{timestamp}.csv")
    
    print(f"Will save output to: {output_file}")
    
    # Process data and generate consolidated file directly
    process_and_save_consolidated_data(skus, machines, output_file)
