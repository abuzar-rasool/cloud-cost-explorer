...
import { Provider, Region, ProviderStats, VMComparisonData, StorageComparisonData, OSType, AccessTier } from '@/types'
...
export async function getProviderStats(): Promise<ProviderStats[]> {
  // Get VM stats
  const vmStats = await prisma.onDemandVMPricing.groupBy({
    by: ['provider_name'],
    _count: {
      id: true,
    },
  ...
  // Get storage stats
  const storageStats = await prisma.storagePricing.groupBy({
    by: ['provider_name'],
    _count: {
      service_name: true,
    },
  ...
  // Get regions per provider
  const regionStats = await prisma.onDemandVMPricing.groupBy({
  ...
  const stats: ProviderStats[] = vmStats.map((vm: VMStatsResult) => {
    const storage = storageStats.find((s: StorageStatsResult) => s.provider_name === vm.provider_name)
...
export async function getTopVMsBySpecs(limit: number = 10) {
  const topCPU = await prisma.onDemandVMPricing.findMany({
    orderBy: {
  ...
  const topMemory = await prisma.onDemandVMPricing.findMany({
    orderBy: {
      memory_gb: 'desc',
    },
    take: limit,
