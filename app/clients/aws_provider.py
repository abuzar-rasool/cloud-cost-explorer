...
class AwsProvider(CloudProviderInterface):
    ...
    async def get_storage_pricing(self, region: Region, specs: StorageSpecs) -> List[StoragePrice]:
        ...
        # Mapping from StorageTier to AWS S3 storage class and product family
        tier_to_aws = {
        ...
        aws_filter = tier_to_aws.get(specs.tier)
        if not aws_filter:
    ...
    async def close(self):
        logger.info("Called AwsProvider.close(). No open connections to close.")
        pass